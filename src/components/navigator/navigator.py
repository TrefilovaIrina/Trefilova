...
class PromptNavigator:
    ...
    def generate_clarifying_questions(self, task: str) -> List[str]:
        ...
        try:
            ...
            prompt = f"""
            Ты — эксперт по формулировке задач для ИИ. Твоя задача — сгенерировать уточняющие вопросы для следующей задачи:
            ...
            4. Вопросы должны быть конкретными и направленными на уточнение:
               - цели пользователя
               - формата результата
               - типа данных
               - ограничений
    ...
    def analyze_task(self, task: str, answers: Dict[str, str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            prompt = f"""
            Ты — архитектор AI‑решений и эксперт по prompt‑engineering.
            ...
            3. Разбей решение на последовательные шаги («steps»). Для каждого:
                ...
                • «llm» — какая LLM оптимальна (GPT‑4, Claude 3 Sonnet, Mistral‑7B‑Instruct и т.д.) с коротким пояснением выбора;  
